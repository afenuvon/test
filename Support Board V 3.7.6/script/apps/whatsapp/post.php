...
if ($raw) {
    ...
    if (($twilio && isset($response['From']) && !$error) || (!$twilio && isset($response['messages']))) {
        ...
        if (!$user) {
            ...
            if (!$twilio) {
                $profile_data = sb_isset(sb_whatsapp_cloud_curl($phone_number_id . '/whatsapp_business_profile?fields=address,email,profile_picture_url,websites', false, $phone_number_id, 'GET'), 'data');
                sb_debug($profile_image);
                $profile_image = sb_whatsapp_cloud_curl($phone_number_id . '/whatsapp_business_profile?fields=address,email,profile_picture_url,websites', false, $phone_number_id);
                sb_debug($profile_image);
            }
