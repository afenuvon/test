            ...
            if (!all && (SB_ADMIN_SETTINGS.dialogflow || SB_ADMIN_SETTINGS.chatbot_features)) {
                ...
                if (SB_ADMIN_SETTINGS.dialogflow) {
                    ...
                    SBF.ajax({
                        ...
                        area.append(response[1] && (!response[5] || !response[5].unknow_answer) ? `<li><div></div><div>${response[1]}</div></li>` : (code ? '' : no_results));
                        icon.sbLoading(false);
                    });
                }
            } else if (!code) {
            ...
            showCreateIntentBox: function (message_id) {
                ...
                if (SBF.isAgent(message.get('user_type'))) {
                    ...
                    if (question && question.payload('sb-human-takeover')) {
                        question = SBChat.conversation.getLastUserMessage(question.get('index'));
                    }
                    if (question) {
                        question = question.payload('translation') && question.payload('translation-language') == SB_ADMIN_SETTINGS.active_agent_language ? question.payload('translation') : question.message
                    }
                    if (message.payload('original-message') && message.payload('original-message-language') == SB_ADMIN_SETTINGS.active_agent_language) {
                        response = message.payload('original-message');
                    } else if (message.payload('translation') && message.payload('translation-language') == SB_ADMIN_SETTINGS.active_agent_language) {
                        response = message.payload('translation');
                    }
                } else {
                    question = message.payload('translation') && message.payload('translation-language') == SB_ADMIN_SETTINGS.active_agent_language ? message.payload('translation') : response;
                    let agent_message = SBChat.conversation.getNextMessage(message.id, 'agent');
                    if (agent_message) {
                        response = agent_message.payload('original-message') && agent_message.payload('original-message-language') == SB_ADMIN_SETTINGS.active_agent_language ? agent_message.payload('original-message') : (agent_message.payload('translation') && agent_message.payload('translation-language') == SB_ADMIN_SETTINGS.active_agent_language ? agent_message.payload('translation') : agent_message.message);
                    }
                }
    ...
    var SBConversations = {
        ...
        openConversation_2: function (message_id, user_type, translation) {
            ...
            if (message) {
                ...
                message.set('translation-language', SB_ADMIN_SETTINGS.active_agent_language);
    ...
    */
    ...
    $(document).ready(function () {
        ...
        */
        ...
        $(admin).on('click', '.sb-profile-list li', function () {
            ...
            switch ($(this).attr('data-id')) {
                ...
                case 'email':
                case 'cc':
                    if (SBChat.conversation.get('source') == 'em') {
        ...
        */
            ...
            switch (id) {
                ...
                case 'close-message':
                    if (settings_area.find('#close-active input').is(':checked') && settings_area.find('#close-chat input').is(':checked')) {
                        infoBottom('Settings > Messages > Close message won\'t work if Settings > Chat > Close chat is active.', 'info');
                    }
                    break;
                case 'chat-timetable':
                case 'follow-message':
        ...
        $(users_table_menu).on('click', '[data-type="online"]', function (e) {
        ...
        */
        ...
        $(flows_area).on('click', '.sb-flow-block', function () {
            ...
            switch (type) {
                case 'start':
                    code = `<div class="sb-title">${sb_('Start event')}</div><div class="sb-setting"><select class="sb-flow-start-select"><option value="message"${block.start == 'message' ? ' selected' : ''}>${sb_('User message')}</option><option value="conversation"${block.start == 'conversation' ? ' selected' : ''}>${sb_('New conversation started')}</option><option value="load"${block.start == 'load' ? ' selected' : ''}>${sb_('On page load')}</option></select></div><div class="sb-title sb-flow-start-textarea${block.start == 'message' ? `` : ` sb-hide`}">${sb_('User message')}</div><div class="sb-setting"><textarea>${block.message}</textarea></div>${code_conditions}<div class="sb-title">${sb_('Disabled')}</div><div class="sb-setting"><input type="checkbox" id="sb-flow-disabled"${block.disabled ? ' checked' : ''}></div>`;
                    break;
                case 'button_list':
        ...
        $(chatbot_qea_repeater).on('click', '.sb-qea-repeater-answer > label > i', function () {
            ...
            if (loading(this)) return;
            SBApps.openAI.rewrite(textarea.val(), false, (response) => {
                $(this).sbLoading(false);
                if (response[0]) {
        ...
        $(chatbot_qea_repeater).on('mouseenter', '.sb-qea-repeater-answer [data-sb-tooltip-init]', function () {
