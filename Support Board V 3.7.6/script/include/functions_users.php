...
function sb_otp($email = false, $code_pairs = false) {
    ...
    return sb_encryption(json_encode([$code_prefix . $code, $email]));
}
...
function sb_update_user($user_id, $settings, $settings_extra = [], $hash_password = true) {
    ...
    if ($first_name) {
        ...
        if (!$last_name && $last_name != 'NULL' && strpos($active_user['last_name'], '#') === 0) {
            $last_name = 'NULL';
        }
    }
...
function sb_set_rating($conversation_id, $user_id, $rating, $message = false, $agent_id = false) {
    ...
    return sb_db_query('INSERT INTO sb_messages(user_id, message, payload, creation_time, status_code, conversation_id) VALUES(' . sb_db_escape($user_id, true) . ', "", "' . sb_db_json_escape($payload) . '", "' . sb_gmt_now() . '", 0, ' . $conversation_id . ')');
}
