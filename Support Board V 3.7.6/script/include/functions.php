...
function sb_get_user_language($user_id = false, $allow_browser_language = false) {
    ...
    if ($user_id && is_numeric($user_id)) {
        ...
        if (!$language && ($allow_browser_language || sb_get_setting('front-auto-translations'))) {
            $language = sb_get_user_extra($user_id, 'browser_language');
        }
    }
    if (!$language && !empty($_POST['language']) && $_POST['language'] != 'false') {
        $language = strtolower(sb_isset($_POST['language'], 0));
    }
    if ($language) {
...
function sb_error($error_code, $function_name, $message = '', $force = false) {
    $message_2 = (sb_is_cloud() ? SB_CLOUD_BRAND_NAME : 'Support Board ') . '[' . $function_name . '][' . $error_code . ']: ' . (is_string($message) ? $message : json_encode($message));
