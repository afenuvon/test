    ...
    if ($sender_id && ($message || $attachments || $is_deleted)) {
        ...
        if ($user_id) {
            ...
            if (!$conversation_id) {
               ...
               if ((isset($response_message['metadata']) && !empty(sb_db_get('SELECT id FROM sb_messages WHERE id = ' . explode('|', $response_message['metadata'])[0])))) {
                    $GLOBALS['SB_FORCE_ADMIN'] = false;
                    return false;
                } else if ($message) {
                    $message_previous = sb_isset(sb_db_get('SELECT message FROM sb_messages WHERE conversation_id = ' . $conversation_id . ' ORDER BY id DESC LIMIT 1'), 'message');
                    if ($message_previous == $message || sb_messenger_rich_messages($message)[0] == $message) {
                        $GLOBALS['SB_FORCE_ADMIN'] = false;
                        return false;
                    }
                }
                if ($count_attachments) {
                    ...
                    $previous_message_count = count(json_decode(sb_isset($previous_message, 'attachments', '[]'), true));
                    if ($previous_message && ($count_attachments == $previous_message_count || $previous_message_count > 1) && strtotime($previous_message['creation_time']) > sb_gmt_now(60, true)) {
                        $GLOBALS['SB_FORCE_ADMIN'] = false;
                        return false;
                    }
