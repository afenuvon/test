...
function sb_update_conversation_status($conversation_id, $status) {
    ...
    if ($status == 3 && $agent && sb_pusher_active() && (sb_get_setting('close-chat') || sb_get_setting('close-ticket') || sb_chatbot_active())) {
    ...
    if ($sender_id) {
        ...
        if ($security || $user_id == sb_get_active_user_ID() || !empty($GLOBALS['SB_FORCE_ADMIN'])) {
            ...
            if ((!$queue_active || (!$is_sender_agent && !sb_agents_online())) && !$is_sender_bot && !$is_human_takeover) {
                ...
                if (($agent_check && sb_get_setting('notify-agent-email')) || (!empty($user['email']) && ($user_check && (sb_get_setting('notify-user-email') || $conversation['source'] == 'em' || sb_get_multi_setting('email-piping', 'email-piping-active'))))) {
    ...
    if (in_array($name, ['registration', 'registration-tickets', 'login', 'login-tickets', 'timetable', 'articles', 'woocommerce-cart'])) {
        ...
        switch ($name) {
            ...
            case 'registration':
                ...
                $code .= '</div>' . $link . '<div class="sb-buttons"><div class="sb-btn sb-submit">' . sb_($visitor ? sb_isset($settings, 'registration-btn-text', 'Create account') : 'Update account') . '</div><div class="sb-btn-text sb-login-area">' . sb_('Sign in instead') . '</div></div>';
                break;
            case 'login-tickets':
            case 'login':
                ...
                $login_tickets = $name == 'login-tickets';
                $password = (!$login_tickets && sb_get_setting('registration-password')) || ($login_tickets && !sb_get_setting('tickets-registration-disable-password')) ? '<div id="password" class="sb-input"><span>' . sb_('Password') . '</span><input autocomplete="false" type="password"></div>' : '<div id="otp" class="sb-input sb-input-btn"><span>' . sb_('One-time code') . '</span><input autocomplete="false" type="text"><div class="sb-btn">' . sb_('Send code') . '</div></div>';
                $code = '<div class="sb-form"><div id="email" class="sb-input"><span>' . sb_('Email') . '</span><input autocomplete="false" type="email"></div>' . $password . '</div><div class="sb-buttons"><div class="sb-btn sb-submit-login">' . sb_('Sign in') . '</div>' . (sb_get_setting('registration-required') == 'login' ? '' : '<div class="sb-btn-text sb-registration-area">' . sb_('Create new account') . '</div>') . '</div>';
                break;
            case 'timetable':
